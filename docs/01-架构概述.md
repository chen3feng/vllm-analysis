...
[PagedAttention](08-PagedAttention-实现分析.md) 将 KV cache 划分为固定大小的 **block（页）**，类似操作系统的虚拟内存分页：
...
[PagedAttention](08-PagedAttention-实现分析.md) 中详述的 KV cache 管理子系统是整个引擎的基石：
...
| Backend | 文件 | 场景 | 特点 |
|---------|------|------|------|
| **FlashAttention** | [flash_attn.py](../vllm/vllm/v1/attention/backends/flash_attn.py) | 默认主路径 | 原生 block_table 支持、cascade attention、FA3 AOT 调度 |
| **Triton** | [triton_attn.py](../vllm/vllm/v1/attention/backends/triton_attn.py) | ALiBi/SWA/FP8 | 纯 Triton、支持 block sparsity |
...
Backend 选择由 [selector.py:52](../vllm/vllm/v1/attention/selector.py#L52) 根据 `head_size`、`dtype`、`block_size`、`use_mla` 等自动决定。
...
| [FlashAttention 实现分析](07-FlashAttention-实现分析.md) | vLLM 默认 attention 后端：版本选择、forward 流程、cascade attention、DCP、与 PagedAttention 集成 |
| [PagedAttention 实现分析](08-PagedAttention-实现分析.md) | Block Table 机制、KV Cache 管理、CUDA/Triton kernel 实现、纯软件 vs GPU 硬件、为何传统引擎没有此机制 |
