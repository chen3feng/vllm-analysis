...
| 入口 | 文件 | 行号 |
|------|------|------|
| 离线推理 | [llm.py](../vllm/vllm/entrypoints/llm.py) | [L194](../vllm/vllm/entrypoints/llm.py#L194) |
| API 服务器（异步） | [async_llm.py](../vllm/vllm/v1/engine/async_llm.py) | [L524](../vllm/vllm/v1/engine/async_llm.py#L524) |
...
[core.py:1294](../vllm/vllm/v1/engine/core.py#L1294) — `EngineCore._handle_client_request()` 分发 `ADD` 类型请求到 `preprocess_add_request()`（[core.py:794](../vllm/vllm/v1/engine/core.py#L794)）：
...
请求的数据结构：[request.py](../vllm/vllm/v1/request.py) — `Request` 对象包含 `request_id`、`prompt_token_ids`、`sampling_params`、`num_computed_tokens`（已计算的 token 数，初始为 0）、`status` 等。
...
[scheduler.py:329](../vllm/vllm/v1/core/sched/scheduler.py#L329) — `Scheduler.schedule()` 是每个 step 的入口：
...
[scheduler.py:364](../vllm/vllm/v1/core/sched/scheduler.py#L364) — 遍历 `self.running` 队列中的每个请求：
...
[scheduler.py:544](../vllm/vllm/v1/core/sched/scheduler.py#L544) — 遍历 `self.waiting` 队列：
...
[output.py:181](../vllm/vllm/v1/core/sched/output.py#L181) — 调度结果包含：
...
[core.py:428](../vllm/vllm/v1/engine/core.py#L428) — `EngineCore.step()` 的调度-执行-采样三步：
...
[gpu_model_runner.py:3955](../vllm/vllm/v1/worker/gpu_model_runner.py#L3955) — 模型执行的核心，按顺序完成 6 个子步骤：

#### 5.1.1 _update_states() — 状态同步

[gpu_model_runner.py:1116](../vllm/vllm/v1/worker/gpu_model_runner.py#L1116)
...
核心数据结构是 `InputBatch`（[gpu_model_runner.py](../vllm/vllm/v1/worker/gpu_model_runner.py)），它在 GPU 端维护 `block_table`、`token_ids`、`positions`、`seq_lens` 等张量。

#### 5.1.2 _prepare_inputs() — 构建输入

[gpu_model_runner.py:1863](../vllm/vllm/v1/worker/gpu_model_runner.py#L1863)
...
[gpu_model_runner.py:2183](../vllm/vllm/v1/worker/gpu_model_runner.py#L2183)
...
`is_prefilling` 的状态判断（[gpu_model_runner.py:2270](../vllm/vllm/v1/worker/gpu_model_runner.py#L2270)：`is_prefilling = num_computed_tokens < num_prompt_tokens`）：
...
[gpu_model_runner.py:3364](../vllm/vllm/v1/worker/gpu_model_runner.py#L3364)
...
[gpu_model_runner.py:3668](../vllm/vllm/v1/worker/gpu_model_runner.py#L3668)
...
模型内部每一层会调用 `Attention.forward()`（[attention.py:437](../vllm/vllm/model_executor/layers/attention/attention.py#L437)），详见第 6 节。
...
`logits_indices` 是每个请求的最后一个 token 在 batch 中的位置索引。公式为 `query_start_loc[1:] - 1`（[gpu_model_runner.py:2135](../vllm/vllm/v1/worker/gpu_model_runner.py#L2135)）。例如 batch 中有 3 个请求，分别调度了 [1, 5, 1] 个 token，`query_start_loc = [0, 1, 6, 7]`，则 `logits_indices = [0, 5, 6]`（取每个请求的最后一个 token）。

### 5.2 执行结果暂存

结果存储在 `ExecuteModelState`（[gpu_model_runner.py:399](../vllm/vllm/v1/worker/gpu_model_runner.py#L399)）：包含 `logits`、`hidden_states`、`sampling_metadata`、`spec_decode_metadata` 等。`execute_model()` 返回 `None`，采样延迟到 `sample_tokens()` 中执行。
...
[attention.py:437](../vllm/vllm/model_executor/layers/attention/attention.py#L437) — 模型每层的 `Attention.forward()` 是 KV cache 读写和 attention 计算的分发点：
...
[flash_attn.py:667](../vllm/vllm/v1/attention/backends/flash_attn.py#L667) — `FlashAttentionImpl.forward()`：
...
[flash_attn.py:850](../vllm/vllm/v1/attention/backends/flash_attn.py#L850) — `FlashAttentionImpl.do_kv_cache_update()`：
...
[gpu_model_runner.py:4319](../vllm/vllm/v1/worker/gpu_model_runner.py#L4319) — `GPUModelRunner.sample_tokens()`：
...
[sampler.py:67](../vllm/vllm/v1/sample/sampler.py#L67) — 核心采样逻辑：
...
[metadata.py](../vllm/vllm/v1/sample/metadata.py) — 包含每个请求的采样参数：`temperature`、`top_p`、`top_k`、随机数生成器 `generators`、惩罚参数、`logits_processors` 等。在 `_prepare_inputs()` 阶段从 `SamplingParams` 构建。
...
[scheduler.py:1283](../vllm/vllm/v1/core/sched/scheduler.py#L1283) — `Scheduler.update_from_output()`：
...
[request.py:315](../vllm/vllm/v1/request.py#L315) — `RequestStatus` 枚举：
...
[output_processor.py:576](../vllm/vllm/v1/engine/output_processor.py#L576) — `OutputProcessor.process_outputs()`：
...
| 组件 | 文件 | 核心行号 |
|------|------|---------|
| LLM 入口 | [llm.py](../vllm/vllm/entrypoints/llm.py) | [L194](../vllm/vllm/entrypoints/llm.py#L194) |
| AsyncLLM | [async_llm.py](../vllm/vllm/v1/engine/async_llm.py) | [L280](../vllm/vllm/v1/engine/async_llm.py#L280), [L524](../vllm/vllm/v1/engine/async_llm.py#L524) |
| EngineCore | [core.py](../vllm/vllm/v1/engine/core.py) | [L428](../vllm/vllm/v1/engine/core.py#L428) (step), [L794](../vllm/vllm/v1/engine/core.py#L794) (add_request), [L1294](../vllm/vllm/v1/engine/core.py#L1294) |
| Scheduler | [scheduler.py](../vllm/vllm/v1/core/sched/scheduler.py) | [L329](../vllm/vllm/v1/core/sched/scheduler.py#L329) (schedule), [L1283](../vllm/vllm/v1/core/sched/scheduler.py#L1283) (update), [L1649](../vllm/vllm/v1/core/sched/scheduler.py#L1649) (_update_request) |
| KVCacheManager | [kv_cache_manager.py](../vllm/vllm/v1/core/kv_cache_manager.py) | [L194](../vllm/vllm/v1/core/kv_cache_manager.py#L194) (get_computed_blocks), [L236](../vllm/vllm/v1/core/kv_cache_manager.py#L236) (allocate_slots), [L429](../vllm/vllm/v1/core/kv_cache_manager.py#L429) (free) |
| GPUModelRunner | [gpu_model_runner.py](../vllm/vllm/v1/worker/gpu_model_runner.py) | [L1116](../vllm/vllm/v1/worker/gpu_model_runner.py#L1116) (_update_states), [L1863](../vllm/vllm/v1/worker/gpu_model_runner.py#L1863) (_prepare_inputs), [L2183](../vllm/vllm/v1/worker/gpu_model_runner.py#L2183) (_build_attention_metadata), [L3668](../vllm/vllm/v1/worker/gpu_model_runner.py#L3668) (_model_forward), [L3955](../vllm/vllm/v1/worker/gpu_model_runner.py#L3955) (execute_model), [L4319](../vllm/vllm/v1/worker/gpu_model_runner.py#L4319) (sample_tokens) |
| Attention 层 | [attention.py](../vllm/vllm/model_executor/layers/attention/attention.py) | [L437](../vllm/vllm/model_executor/layers/attention/attention.py#L437) |
| FlashAttention 后端 | [flash_attn.py](../vllm/vllm/v1/attention/backends/flash_attn.py) | [L388](../vllm/vllm/v1/attention/backends/flash_attn.py#L388) (MetadataBuilder), [L667](../vllm/vllm/v1/attention/backends/flash_attn.py#L667) (forward), [L850](../vllm/vllm/v1/attention/backends/flash_attn.py#L850) (do_kv_cache_update) |
| Sampler | [sampler.py](../vllm/vllm/v1/sample/sampler.py) | [L67](../vllm/vllm/v1/sample/sampler.py#L67) (forward), [L238](../vllm/vllm/v1/sample/sampler.py#L238) (sample), [L366](../vllm/vllm/v1/sample/sampler.py#L366) (apply_logits_processors) |
| OutputProcessor | [output_processor.py](../vllm/vllm/v1/engine/output_processor.py) | [L576](../vllm/vllm/v1/engine/output_processor.py#L576) |
| 请求数据结构 | [request.py](../vllm/vllm/v1/request.py) | RequestStatus [L315](../vllm/vllm/v1/request.py#L315) |
