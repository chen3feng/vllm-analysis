...
所有非稳定 ABI 的 C++/CUDA 算子都在 [torch_bindings.cpp](vllm/csrc/torch_bindings.cpp) 中注册。这个文件约 450 行，是整个 C++ 层的注册总线。
...
TORCH_LIBRARY_EXPAND(_C, ops) {
  ops.def(
      ...
      "   Tensor? prefix_offsets, Tensor? causal_diag) -> ()");
...
`TORCH_LIBRARY_EXPAND` 定义在 [registration.h:13](vllm/csrc/core/registration.h#L13)：
...
|--------|-----------|----------|-------------|
| `vllm._C` | `_C` [CMakeLists.txt:604](vllm/CMakeLists.txt#L604) | `torch.ops._C.*` | paged_attention, cutlass_scaled_mm, rms_norm, rotary_embedding, silu_and_mul, 量化 kernel 等 |
...
| `vllm._moe_C` | `_moe_C` [CMakeLists.txt:1260](vllm/CMakeLists.txt#L1260) | `torch.ops._moe_C.*` | MoE 专用 kernel |
| `vllm._rocm_C` | `_rocm_C` [CMakeLists.txt:1281](vllm/CMakeLists.txt#L1281) | `torch.ops._rocm_C.*` | ROCm 专用 kernel |

### 3.4 Python 端触发加载

当 Python 执行 `current_platform.import_kernels()`（[interface.py:242-249](vllm/vllm/platforms/interface.py#L242)）时：
...
`REGISTER_EXTENSION` 宏（[registration.h:22-27](vllm/csrc/core/registration.h#L22)）在模块导入时创建 `PyInit_<name>()` 函数，其执行会运行所有 `TORCH_LIBRARY` 块中注册的算子。

### 3.5 Python 封装层：`_custom_ops.py`

[_custom_ops.py](vllm/vllm/_custom_ops.py)（约 3900 行）为所有 C++ 算子提供 Python 接口。它做以下事情：
...
典型模式（[_custom_ops.py:2599](vllm/vllm/_custom_ops.py#L2599)）：
...
- [csrc/ops.h:18+](vllm/csrc/ops.h#L18) — paged_attention_v1, paged_attention_v2, rms_norm, rotary_embedding, silu_and_mul 等
- [csrc/cache.h:9+](vllm/csrc/cache.h#L9) — reshape_and_cache, swap_blocks, copy_blocks 等
...
[libtorch_stable/torch_bindings.cpp:9-543](vllm/csrc/libtorch_stable/torch_bindings.cpp#L9) — 注册在 `torch.ops._C.*` 命名空间下（和非稳定 ABI 完全相同！）：
...
核心基础设施在 [torch_utils.py:927-969](vllm/vllm/utils/torch_utils.py#L927)：
...
`dispatch_key` 由平台决定：
- NVIDIA GPU → `"CUDA"`（[platforms/cuda.py:166](vllm/vllm/platforms/cuda.py#L166)）
- CPU → `"CPU"`（[platforms/cpu.py:46](vllm/vllm/platforms/cpu.py#L46)）
- XPU → `"XPU"`
...
| 算子 | 定义位置 | 行号 |
|------|---------|------|
| `torch.ops.vllm.unified_attention_with_output` | [attention.py:734-761](vllm/vllm/model_executor/layers/attention/attention.py#L734) | 注册在 [L777-781](vllm/vllm/model_executor/layers/attention/attention.py#L777) |
| `torch.ops.vllm.unified_kv_cache_update` | [attention.py:725-729](vllm/vllm/model_executor/layers/attention/attention.py#L725) | 注册在 [L725-729](vllm/vllm/model_executor/layers/attention/attention.py#L725) |
| `torch.ops.vllm.maybe_calc_kv_scales` | [attention.py:636-639](vllm/vllm/model_executor/layers/attention/attention.py#L636) | — |
| `torch.ops.vllm.fused_rope_and_unified_kv_cache_update` | [rope_kvcache_fusion.py:85-89](vllm/vllm/compilation/passes/fusion/rope_kvcache_fusion.py#L85) | 编译优化 pass |
| `torch.ops.vllm.rocm_aiter_*` (30+ ops) | [_aiter_ops.py](vllm/vllm/_aiter_ops.py) | ROCm aiter 库的算子封装 |
...
在 attention backend 中（如 [triton_attn.py:638-670](vllm/vllm/v1/attention/backends/triton_attn.py#L638)）：
...
|----------------|------|------|------|
| `unified_attention` | [triton_unified_attention.py](vllm/vllm/v1/attention/ops/triton_unified_attention.py) | [L763](vllm/vllm/v1/attention/ops/triton_unified_attention.py#L763) | 统一的 prefill + decode attention |
| `decode_attention_fwd` | [triton_decode_attention.py](vllm/vllm/v1/attention/ops/triton_decode_attention.py) | [L729](vllm/vllm/v1/attention/ops/triton_decode_attention.py#L729) | 仅 decode 阶段的 attention |
| `context_attention_fwd` | [triton_prefill_attention.py](vllm/vllm/v1/attention/ops/triton_prefill_attention.py) | [L191](vllm/vllm/v1/attention/ops/triton_prefill_attention.py#L191) | 仅 prefill 阶段的 attention |
| `triton_reshape_and_cache_flash` | [triton_reshape_and_cache_flash.py](vllm/vllm/v1/attention/ops/triton_reshape_and_cache_flash.py) | [L319](vllm/vllm/v1/attention/ops/triton_reshape_and_cache_flash.py#L319) | KV cache 写入 |
...
vLLM 有 Rust 代码，但**不参与算子注册**。Rust 代码是一个独立的前端二进制程序 `vllm-rs`（[Cargo.toml](vllm/rust/src/cmd/Cargo.toml)），在 Python 侧由 [v1/utils.py:240](vllm/vllm/v1/utils.py#L240) 启动。它包括：
...
[custom_op.py:103-353](vllm/vllm/model_executor/custom_op.py#L103) — 对于**跨平台有多个实现**的高层算子（如 fused_moe、量化线性层），vLLM 提供了 `CustomOp` 基类：
...
| 组件 | 文件 | 核心行号 |
|------|------|---------|
| C++ op 注册总线 | [torch_bindings.cpp](vllm/csrc/torch_bindings.cpp) | [L18-445](vllm/csrc/torch_bindings.cpp#L18) |
| C++ registration 宏 | [registration.h](vllm/csrc/core/registration.h) | [L13](vllm/csrc/core/registration.h#L13) (EXPAND), [L22](vllm/csrc/core/registration.h#L22) (REGISTER_EXTENSION) |
| C++ op 头文件 | [ops.h](vllm/csrc/ops.h) | [L18+](vllm/csrc/ops.h#L18) |
| C++ cache op 头文件 | [cache.h](vllm/csrc/cache.h) | [L9+](vllm/csrc/cache.h#L9) |
| 稳定 ABI 注册 | [libtorch_stable/torch_bindings.cpp](vllm/csrc/libtorch_stable/torch_bindings.cpp) | [L9-543](vllm/csrc/libtorch_stable/torch_bindings.cpp#L9) |
| Python op 封装 | [_custom_ops.py](vllm/vllm/_custom_ops.py) | 约 3900 行 |
| Python op 注册工具 | [torch_utils.py](vllm/vllm/utils/torch_utils.py) | [L927-969](vllm/vllm/utils/torch_utils.py#L927) |
| Attention 算子分发点 | [attention.py](vllm/vllm/model_executor/layers/attention/attention.py) | [L437](vllm/vllm/model_executor/layers/attention/attention.py#L437) (forward), [L725-729](vllm/vllm/model_executor/layers/attention/attention.py#L725) (kv_cache_update), [L734-761](vllm/vllm/model_executor/layers/attention/attention.py#L734) (unified_attention) |
| FlashAttention backend | [flash_attn.py](vllm/vllm/v1/attention/backends/flash_attn.py) | [L667](vllm/vllm/v1/attention/backends/flash_attn.py#L667) (forward) |
| Triton backend | [triton_attn.py](vllm/vllm/v1/attention/backends/triton_attn.py) | [L638-670](vllm/vllm/v1/attention/backends/triton_attn.py#L638) |
| Triton unified attention kernel | [triton_unified_attention.py](vllm/vllm/v1/attention/ops/triton_unified_attention.py) | [L179](vllm/vllm/v1/attention/ops/triton_unified_attention.py#L179) (kernel), [L763](vllm/vllm/v1/attention/ops/triton_unified_attention.py#L763) (wrapper) |
| Platform import_kernels | [interface.py](vllm/vllm/platforms/interface.py) | [L242-249](vllm/vllm/platforms/interface.py#L242) |
| CustomOp 平台分发 | [custom_op.py](vllm/vllm/model_executor/custom_op.py) | [L103-353](vllm/vllm/model_executor/custom_op.py#L103) |
| CMake 构建 | [CMakeLists.txt](vllm/CMakeLists.txt) | [L604](vllm/CMakeLists.txt#L604) (_C), [L1037](vllm/CMakeLists.txt#L1037) (_C_stable_libtorch), [L1260](vllm/CMakeLists.txt#L1260) (_moe_C) |
| Rust 入口 | [Cargo.toml](vllm/rust/src/cmd/Cargo.toml) | — |
